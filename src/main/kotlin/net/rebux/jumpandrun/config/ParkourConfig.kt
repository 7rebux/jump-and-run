...
object ParkourConfig : CustomConfiguration("parkour.yml") {
    ...
    internal object Block {
        val finish =
            Material.getMaterial(config.getString("block.finish") ?: "GOLD_BLOCK")
                ?: error("Material not found for finish block!")
        val reset =
            Material.getMaterial(config.getString("block.reset") ?: "REDSTONE_BLOCK")
                ?: error("Material not found for reset block!")
        val checkpoint =
            Material.getMaterial(config.getString("block.checkpoint") ?: "IRON_BLOCK")
